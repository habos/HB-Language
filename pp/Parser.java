...
public class Parser extends Lexeme{
	...
	public Lexeme optElse(Lexeme env) throws IOException{
		if(check(Type.ELSE)){
			if(blockPending()){
				return cons(Type.ELSE, null, block(env));
			}else{
				return cons(Type.ELSE, null, ifStat(env));
			}
		}else{
			return null;
		}
	...
	private boolean unaryPending(){
		return check(Type.BOOLEAN) || check(Type.VARIABLE) || check(Type.NUMBER) || check(Type.OPAREN) || check(Type.STRING) || arrayPending() || functionCallPending();
	}
